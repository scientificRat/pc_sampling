...
def triangles_to_pc(triangles):
    with tf.device('/gpu:1'):
        ...
        reduced_sample = gather_point(pt_sample, farthest_point_sample(2048, pt_sample))
        print reduced_sample
    with tf.Session('') as sess:
        ret = sess.run(reduced_sample)
    print ret.shape, ret.dtype
    return ret.reshape(ret.shape[1], ret.shape[2])
...
if __name__ == '__main__':
    ...
    triangles = read_off('/repository/ModelNet40/airplane/test/airplane_0627.off')
